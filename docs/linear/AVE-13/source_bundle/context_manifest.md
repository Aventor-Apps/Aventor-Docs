...
- UI scope is limited to required Google/Meta integration/connect/select recovery. Conversion tracking, Meta Pixel, GBP, and switch/continue/pause questions are Launch Agent conversational/structured-decision behavior, not new readiness cards/panels. If provider setup information touches the right-side panel, match the existing light `MainStudio`/`DesignPanel` media/info panel style. Use local Open Design only for substantive provider-integration UI beyond existing patterns; if Open Design is unavailable or cannot provide a needed artifact, block that UI phase as `BLOCKED_DESIGN`.
